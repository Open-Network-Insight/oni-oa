    ...
    def init_dicts(self):
        if os.path.isfile(self._qclass_file_path):
            with open(self._qclass_file_path, 'rb') as qclass_file:
                ...
                self.qclass_dict.update(d1)
                self.qclass_dict.update(d2)
        if os.path.isfile(self._qtype_file_path):
            with open(self._qtype_file_path, 'rb') as qtype_file:
                ...
                self.qtype_dict = dict([(x[1],x[0]) for x in qtype_rows])
        if os.path.isfile(self._rcode_file_path):
            with open(self._rcode_file_path) as rcode_file:
                ...
                self.rcode_dict = dict([(x[0],x[1]) for x in rcode_rows])

    def get_name(self,key,column): 
        if column == COL_CLASS:
            if key in self.qclass_dict:
                return self.qclass_dict[key]
            else:
                return key
        if column == COL_QTYPE:
            if key in self.qtype_dict:
                return self.qtype_dict[key]
            else:
                return key
        if column == COL_RCODE:
            if key in self.rcode_dict:
                return self.rcode_dict[key]
            else:
                return key
