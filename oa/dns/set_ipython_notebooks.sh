...
if [[ -n "$oni_oa_path" ]]; then
    ...
    if [[ ! -d "$user_path/$new_path" ]]; then
	log_info "Creating today's directory"
	`mkdir $user_path/${new_path}`
    fi
    `cp ${master_path}/${EDGE} ${user_path}/${new_path}/${CPEDGE}`
    `cp ${master_path}/${THREAT} ${user_path}/${new_path}/${CPTHREAT}`
    log_info "Create ipython notebooks completed for date ${new_path}"
    exit 0
fi
